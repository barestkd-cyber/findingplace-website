/* ─────────────────────────────────────────────
   ...
   Renders inline wherever [data-tour-inline] exists (contact + home),
   and as a modal from any [data-tour-open] button on the other pages.
...
(function () {
  ...
  var state = fresh();
  var modal, dialog, inlineHost, lastFocused;

  function fresh() {
    return { path: null, picked: [], slot: null, weekOffset: 0 };
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    inlineHost = document.querySelector("[data-tour-inline]");
    if (inlineHost) {
      inlineHost.innerHTML = '<div class="tour-body"></div>';
      dialog = inlineHost;
      renderStart();
    }
    document.addEventListener("click", function (e) {
      ...
      if (!t) return;
      e.preventDefault();
      // Already on the page? Scroll to it rather than running a second copy.
      if (inlineHost) {
  ...
  function openModal(trigger) {
    ...
    dialog = modal.querySelector(".tour-dialog");
    state = fresh();
  ...
  function setBody(html) { dialog.querySelector(".tour-body").innerHTML = html; }
  ...
  function renderStart() {
    ...
    dialog.querySelectorAll("[data-path]").forEach(function (b) {
  ...
  function renderPrograms() {
    ...
    function refresh() {
      dialog.querySelectorAll("[data-key]").forEach(function (b) {
        b.setAttribute("aria-checked", state.picked.indexOf(b.getAttribute("data-key")) >= 0 ? "true" : "false");
      });
      dialog.querySelector(".tour-next").disabled = state.picked.length === 0;
    }
    dialog.querySelectorAll("[data-key]").forEach(function (b) {
      b.addEventListener("click", function () {
    ...
    dialog.querySelector(".tour-back").addEventListener("click", renderStart);
    dialog.querySelector(".tour-next").addEventListener("click", renderIntake);
    refresh();
  }

  /* ---- step 2: intake ------------------------------------------------- */
  function field(name, label, required, type) {
    return '<div class="form-group"><label for="tf_' + name + '">' + esc(label) +
           (required ? ' *' : '') + '</label>' +
           '<input id="tf_' + name + '" name="' + name + '" type="' + (type || "text") + '"></div>';
  }
  ...
  function renderIntake() {
    ...
    html += '<div class="form-row single"><div class="form-group">' +
            '<label for="tf_notes">Anything you&rsquo;d like us to know?</label>' +
            '<textarea id="tf_notes" name="notes" placeholder="Questions, schedule needs, how you heard about us..."></textarea>' +
            '</div></div>';
    html += '<div class="tour-actions">' +
              '<button class="btn btn-outline tour-back" type="button">Back</button>' +
    ...
    dialog.querySelector(".tour-form").addEventListener("submit", function (e) {
      e.preventDefault();
      submit(e.target);
    });
  }
  ...
  function renderSuccess(lead) {
    ...
    html += '<div class="tour-actions tour-actions--center">' +
              ...
              (modal && !modal.hasAttribute("hidden") ? '<button class="btn btn-primary" type="button" data-tour-close>Done</button>' : '') +
            '</div>';
    html += '</div>';
    setBody(html);
    dialog.querySelector(".tour-restart").addEventListener("click", renderStart);
  }
})();
